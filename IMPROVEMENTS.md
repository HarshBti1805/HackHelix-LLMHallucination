...
| A.12 | Commit: `"document upload + audit with dedicated report view"` | ✅ Committed as `1fef5bb`. |
...
| # | Task | Status |
|---|---|---|
| B.1 | `npm install @anthropic-ai/sdk`. Add `ANTHROPIC_API_KEY` to `.env.example` and `.env.local`. | ✅ Installed `@anthropic-ai/sdk@^0.90.0`. `.env.example` documents the locked-auditor exemption (chat-only) + the no-free-tier reality of the Anthropic API and explains the Haiku hardcoding rationale. `.env.local` placeholder added (empty — user fills in). |
| B.2 | Create `lib/providers/anthropic.ts` mirroring `lib/providers/gemini.ts` — exports `anthropicChat(messages, model)`. Map roles: Anthropic uses `user` / `assistant`, same as OpenAI. System messages passed via `system` parameter, extracted from the messages array. | ✅ Mirrors `gemini.ts` shape (lazy client, system extracted into top-level param). Hardcoded to `claude-haiku-4-5` (rolling alias verified from official model overview). Anthropic-specific quirks handled internally: required `max_tokens` (4096 default), text-block array → string concatenation. `types.ts` `ChatModel` updated: `claude-3-5-sonnet-latest` and `claude-3-5-haiku-latest` removed; `claude-haiku-4-5` is the only Claude entry. |
| B.3 | In `app/api/chat/route.ts`, add a case for `provider === "anthropic"` dispatching to `anthropicChat`. | ✅ Replaced the if/else with an exhaustive `switch (provider)`; new `"anthropic"` arm calls `anthropicChat(messages, model as AnthropicChatModel)`. `isValidProvider` extended to accept `"anthropic"`. `tsc --noEmit` clean. |
| B.4 | In the chat UI provider switcher, add Anthropic as a third option, hardcoded to `claude-haiku-4-5` (no sub-dropdown — same single-model pattern as Gemini Flash; deviates from the original "default Sonnet" plan, decided at Phase B prep for rate-limit reliability and per-token cost over the eval's hundreds of upstream calls). | ✅ Removed the `WiredProvider` narrowing alias (its job — quarantining unwired providers — is done). `PROVIDER_MODELS` and `PROVIDER_LABEL` now exhaustive against the full `Provider` union; compiler will fail loudly if a future provider is added to `types.ts` without a switcher entry. Anthropic option renders as "Anthropic / claude-haiku-4-5" using the existing single-model collapse path (model column shows static text, no purposeless one-item dropdown). Cleaned up 6 redundant `as Provider` casts that were leftover scaffolding. `tsc --noEmit` clean. |
| B.5 | Quick smoke test in the chat: send a trivial prompt via Anthropic, confirm response comes back and gets audited normally. No further integration work. | ✅ Trivial prompt ("What is 2+2?"): `/api/chat` 200 in 3.5s → "2 + 2 = 4"; `/api/audit` 200 in 10.8s → 1 claim, consensus `verified` (one agent disagreement on confidence, expected for a definitional claim with no Tavily hits). Substantive prompt (Eiffel Tower height + city in one sentence): `/api/chat` 200 in 3.8s; `/api/audit` 200 in 15.6s extracted 3 atomic claims (location, meters, feet), all `verified` 3/3, no false hallucinations. Audit pipeline is provider-agnostic — confirmed it sees only response text, not provenance. |
| B.6 | Create `eval/prompts.json` — 15 prompts with ground-truth labels. Use the template in the "Eval test set" section below. | ✅ 15 prompts, 3 each across all 5 categories (`fabricated-citation`, `specific-fact`, `contested-claim`, `compound-claim`, `open-research`); unique IDs validated. Two reasoned deviations from the IMPROVEMENTS suggestions: (a) `fact-01` swapped from "IRA year" to a Fahrenheit prompt (the IRA-2021 claim is the documented consensus-misses-it failure mode in `renewable-energy-brief.md`, would muddy the eval signal); (b) `cite-03` rewrote the MIT longitudinal-study prompt to make the chronological impossibility (longitudinal study finishing the same year ChatGPT launched) more diagnostic. Both approved by user before B.7. |
| B.7 | Create `scripts/eval.ts` — a Node/tsx script. Reads `eval/prompts.json`. For each prompt, runs it through all three providers (sequential to avoid rate limits; use the existing `openaiChat`, `geminiChat`, `anthropicChat` wrappers). For each response, runs `auditDocument(response, filename, { maxClaims: 25 })` (treats the response as a one-off document). Records results. | ✅ Standalone tsx script. Manually loads `.env.local` then `.env` (first-set-wins) since tsx doesn't share Next.js's auto-load. Sequential per-prompt and per-provider as specified — no fan-out, single in-flight upstream call at a time. Per-cell try/catch records errors as `{ error }` rather than crashing the run (critical for a 15×3 run). `--prompt <id>[,<id>...]` flag for dry-run mode; dry-run output suffixed (e.g. `results.dryrun-cite-01.json`) so it doesn't clobber a full-run file. |
| B.8 | Script aggregates per-provider hallucination counts and emits two files: `eval/results.json` (full records) and `eval/results.md` (markdown summary table + per-prompt breakdown). | ✅ Aggregator embedded in `scripts/eval.ts` (single tool, single source of truth — no separate aggregator-only script to drift). `eval/results.json` contains generated_at + prompts + every cell (raw response, full DocumentAudit, error, wall-clock). `eval/results.md` has three sections: headline summary table (provider × claim-counts × hallucination_rate), per-category breakdown (5 sub-tables), per-prompt detail (prompt + ground-truth notes + per-provider verdict spread + wall-clock). Hallucination rate formula matches IMPROVEMENTS.md "Hallucination rate metric": `(contradicted + likely_hallucination) / total_claims`, dash for divide-by-zero. |
| B.9 | Run the eval: `npx tsx scripts/eval.ts`. This will take 10–20 minutes and several dollars of API spend. Let it run while you work on other things; it prints progress to console. | ⚠️ Completed in 1257.4s (~21 min). OpenAI 15/15 + Anthropic 15/15 fully covered, no errors. **Gemini hit free-tier daily quota wall after 8 cells** — `429 Too Many Requests` with `limit: 20 requests/day` on `gemini-2.5-flash`. Failed cells (preserved in `results.json` with full SDK error payloads for transparency): all 3 `compound-claim`, all 3 `open-research`, and `contested-03`. Gemini headline rate (26.9%) is technically computed correctly but systematically inflated — the missing categories are exactly where the other two providers scored 0% / ≤4.3%. Two Anthropic cells (`fact-03`, `open-01`) had 311s / 326s wall-clock — diagnosed as audit-pipeline parallelism (many Tavily + verifier calls fanning out from verbose bulleted responses), not chat-model latency. Decision (with user): publish as-is with prominent coverage disclosure rather than re-running Gemini 24h later — methodological cost of mixing timing conditions exceeds the missing-cell cost. |
| B.10 | Copy the markdown summary table from `eval/results.md` into `README.md` under a new "Empirical model comparison" section. Add a short methodology paragraph below the table: how many prompts, what kind, who audited, what the metric means. | ✅ Section added between "Document audit findings" and "Future work". Did NOT use the headline aggregate from `results.md` (would obscure the per-category structure where the actual finding lives). Structure (per user spec): lead paragraph → upfront coverage disclosure (not a footnote) → per-category breakdown table (15 rows, one per category × provider, `—†` for the 6 errored Gemini cells with single explanatory footnote) → 4 numbered qualitative findings (specific-fact 0% across all = auditor-reliability signal; refusal-vs-fabrication invisible to rate metric; Anthropic verbose hedging deflates per-claim rate; Gemini cite-03 = second observed adjacent-topic-source false-verified case) → methodology paragraph (verbatim agreed wording: "rate-limit reliability and per-token cost"). Also fixed 4 stale README references inline (lead paragraph + "What it does" #1 still said two providers; Stack section missing `@anthropic-ai/sdk`; env var table missing `ANTHROPIC_API_KEY`; architecture file tree + data-flow diagram still said "OpenAI or Gemini") + 6 corresponding ARCHITECTURE.md references (§1 model table, §2 directory tree, §3 type unions, §4 data flow switch, §6 /api/chat schema, plus new §5.2a `lib/providers/anthropic.ts` section mirroring §5.2's structure and depth). Self-consistency: a commit publishing a three-provider eval shouldn't ship docs that describe two providers. |
| B.11 | Add a sentence to `README.md`'s "Known limitations" section: eval set is small (N=15), results are suggestive not conclusive, per-provider rates would need a much larger and more diverse prompt set for confident ranking. | ✅ Added two new bullets to "Known limitations" rather than one — the "small N" caveat is implicit in the methodology paragraph's "results are suggestive, not a ranking", and the actual sharp-edged caveats from the run are different: (1) "Three-provider eval coverage is uneven" (Gemini's 7/15 errored cells, full-coverage comparison is OpenAI vs Anthropic only); (2) "Hallucination rate favors verbose providers" (hedging into many `unverified_plausible` claims artificially deflates the per-claim rate; refusal = undefined rate; `specific-fact` and `compound-claim` are the fairest comparisons). Also appended one verbatim sentence (per user spec) to the existing "Strict date/year verification can be outvoted" entry recording the cross-phase pattern: "Observed twice across Phase A and Phase B: the Defender's charitable reading plus Tavily returning adjacent-topic sources can produce a false-verified verdict on a fabricated claim." |
