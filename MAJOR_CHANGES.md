...
- [x] **#8 Citation & reference checker** — `lib/citations.ts` + `lib/bib.ts` (Crossref + Semantic Scholar) + `lib/prompts/citation.ts` + `POST /api/check-citations` + `/citations` page. Extracts references and verifies each against structured scholarly indices; fabricated citations surface as "Not found".
...
Docs synced: `CLAUDE.md` file-responsibilities table + sanctioned-extensions note updated. New scholarly-index calls (`lib/bib.ts`) and optional contact email read `CROSSREF_CONTACT_EMAIL` / `SEMANTIC_SCHOLAR_API_KEY` from env (both optional).
